        ...
        return create((err, data) => {
          ...
          fs.writeFile(path, str, {
            ...
            if(err) {
              ...
              if(err.code === 'EEXIST') {
                return _readOrCreateJson({path, create, loaded, stored});
              }
            }
...
api.proofs = (operation, callback) => {
  ...
  didv1.attachInvocationProof({
    operation,
    // FIXME the ID of the capability that is being invoked (e.g. the ID of
    // the record in the operation for self-invoked capabilities).
