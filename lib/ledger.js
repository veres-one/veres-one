...
async function _setupGenesisNode() {
  try {
    ...
    const didv1 = new (require('did-veres-one')).VeresOne({
      ...
      // FIXME: what flag is used to determine test vs live?
      // dev mode validator does not want `test` DIDs
      mode: 'live',
    });
    ...
    await _sendElectorPool({maintainerDoc});
...
async function _sendElectorPool({maintainerDoc}) {
  const method = maintainerDoc.getVerificationMethod(
