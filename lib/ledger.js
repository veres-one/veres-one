    ...
    async.auto({
      ledgerOwner: callback => {
        ...
        const adminIdentity = {
          ...
          type: 'Identity',
          sysSlug: 'admin',
          label: 'Administrator Account for Veres One Peer Node',
          email: 'admin@' + config.server.domain,
          sysPassword: config['veres-one'].adminPassword,
          sysPublic: ['label', 'url', 'description'],
          sysResourceRole: [{
          ...
          url: config.server.baseUri,
          description: 'Administrator Account for Veres One Peer Node'
        };
        brIdentity.insert(null, adminIdentity, callback);
      },
      ...
      ledgerNode: ['genesisBlock', 'ledgerOwner', (results, callback) => {
        ...
        console.log("BLOCKS", JSON.stringify(genesisBlock, null , 2));
