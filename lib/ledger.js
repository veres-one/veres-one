...
function _createGenesisBlock(callback) {
  async.auto({
    ...
    storeDdos: ['maintainerDdo', 'governorsDdo', (results, callback) =>  {
      fs.writeFileSync(config['veres-one'].maintainerConfigFile,
        JSON.stringify(results.maintainerDdo.privateDidDocument, null, 2), 
        'utf-8');
      fs.writeFileSync(config['veres-one'].governorsConfigFile,
        JSON.stringify(results.governorsDdo.privateDidDocument, null, 2),
        'utf-8');
      callback();
    }],
    getPrivateKey: ['storeDdos', (results, callback) =>
