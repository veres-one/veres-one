...
async function setupVeresOneLedger() {
  ...
  await bedrock.runOnce('veres-one.setupLedger', promisify(setup));
...
function _getLedgerOwner(options, callback) {
  ...
  brAccount.getCapabilities({id}, (err, account) => {
    if(err) {
      return callback(err);
    }
    ...
    brAccount.insert(
      {actor: null, account: adminAccount, meta}, err => {
        if(err) {
          return callback(err);
        }
        brAccount.getCapabilities({id}, callback);
      });
  });
}
