...
async function _setupGenesisNode() {
  try {
    ...
    let method = maintainerDoc.getVerificationMethod(
    ...
    method = maintainerDoc.getVerificationMethod(
      {proofPurpose: 'assertionMethod'});
    const capabilityInvocationKey = maintainerDoc.keys[method.id];
    await _sendElectorPool(
      {maintainerDid: maintainerDoc.doc.id, capabilityInvocationKey});
  } catch(error) {
...
async function _sendElectorPool({maintainerDid, capabilityInvocationKey}) {
  ...
  const electorPoolDoc = {
    '@context': didContexts,
    id: config['veres-one'].electorPool,
    type: 'ElectorPool',
  ...
  const signedOperation = await jsigs.sign(operation, {
    ...
    // FIXME: is this supposed to be a capability invocation at all?
    // and if so, what is the capability, using assertionMethod did not
    // line up with validator
  ...
  logger.debug('ElectorPool operatation.', {operation: signedOperation});
