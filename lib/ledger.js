...
async function _sendElectorPool({electorPoolId, maintainerDoc}) {
  ...
  // gets the ledger id from the target
  const getLedgerId = ({client = {}} = {}) => {
    const {
      ledgerAgentStatus: {
        latestConfigEvent: {
          ledgerConfiguration: {
    ...
    throw new Error('Unable to determine ledger for current target');
  };
  ...
  operation.proof = {
    ...
    invocationTarget: getLedgerId(target),
