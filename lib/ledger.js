...
// add routes
bedrock.events.on('bedrock.init', () => bootstrapLedger());

function bootstrapLedger() {
  const brEvent = config.ledger.jobs.scheduleConsensusWork.enabled ?
    'bedrock-server.readinessCheck' : 'bedrock.started';
  logger.debug(`AAAA - Bedrock Event: ${brEvent}`);

  bedrock.events.on(brEvent, async () => {
    let ledgerAgent;
    while(!ledgerAgent) {
...
async function _findAgent() {
  ...
  let iterator;
  try {
    logger.debug('AAAA - Get Agent Iterator');
...
async function _setupGenesisNode() {
  try {
    ...
    // dangerously presumes that the key is an Ed25519VerificationKey2018 which
    // is only safe momentarily.
    const assertionMethodKey = new Ed25519VerificationKey2018(
    ...
    logger.debug('AAAA - brLedgerAgentAdd', {ledgerNode});

    // create the maintainer's DID on the ledger
    logger.debug('Attempting to register the Maintainer\'s DID.');
    await pRetry(async () => didv1.register({didDocument: maintainerDoc}), {
      retries: 30,
