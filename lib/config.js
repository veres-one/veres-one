...
require(path.join(config.paths.config, 'stats'));
...
// the hostnames that will correspond to the witnesss in the witnessPool
// dev mode defaults to local genesis node.
config['veres-one'].witnessHosts = [config.server.host];
...
scheduleConsensusWork.enabled = true;
// 2 minutes (120000 ms)
...
cc('veres-one.config', () => ({
  ...
  witnessSelectionMethod: {
    ...
    // the witness pool document ID is configured in ledger.js
    witnessPool: null
  },
  sequence: 0,
