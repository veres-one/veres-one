...
require(path.join(config.paths.config, 'jobs'));
require(path.join(config.paths.config, 'redis'));
require(path.join(config.paths.config, 'https-agent'));
require(path.join(config.paths.config, 'continuity'));
...
// NOTE: this setting should be set to true in production
config['https-agent'].rejectUnauthorized = false;

// the hostnames that will correspond to the electors in the electorPool
// dev mode defaults to local genesis node.
...
const {scheduleConsensusWork} = config.ledger.jobs;
scheduleConsensusWork.enabled = SCHEDULE_CONSENSUS_WORK !== 'false';
scheduleConsensusWork.workSessionConcurrencyPerInstance = 1;
// 2 minutes (120000 ms)
scheduleConsensusWork.workSessionGracePeriod = 120000;
...
cc('veres-one.config', () => ({
  ...
  electorSelectionMethod: {
    ...
    // this is configured in ledger.js
    electorPool: null,
  },
  sequence: 0,
