...
// add schema.org context to constants
config.constants.SCHEMA_CONTEXT_URL = 'https://schema.org/';
constants.CONTEXTS[constants.SCHEMA_CONTEXT_URL] = JSON.parse(
  fs.readFileSync(
...
// add veres context to constants
config.constants.VERES_CONTEXT_V1_URL = 'https://w3id.org/veres/v1';
constants.CONTEXTS[constants.VERES_CONTEXT_V1_URL] = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, '../contexts/veres-v1.jsonld'),
    {encoding: 'utf8'}));
...
config.views.vars['bedrock-angular-footer'].copyright =
  'Copyright © 2017 Digital Bazaar, Inc. All rights reserved.';

config['veres-one'] = {};
config['veres-one'].did = 'did:v1:ledger';
cc('veres-one.config', () => ({
...
cc('veres-one.ddoPublicKey', () => ({
  '@context': 'https://w3id.org/identity/v1',
  id: config['veres-one'].did + '/keys/1',
  type: 'CryptographicKey',
  owner: config['veres-one'].did,
  label: 'Signing Key for ' + config['veres-one'].did,
  publicKeyPem: config['veres-one'].publicKeyPem
}));

cc('veres-one.ddo', () => ({
  '@context': 'https://w3id.org/identity/v1',
  id: config['veres-one'].did,
  publicKey: [config['veres-one'].ddoPublicKey]
}));
